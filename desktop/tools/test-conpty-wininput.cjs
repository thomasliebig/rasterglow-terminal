...
setTimeout(()=>{const key=/KEY 112 59 1 0 0/.test(out);const click=/MOUSE 17 9 1 0/.test(out);const wheel=/MOUSE 17 9 4286578688 4/.test(out);console.log(`F1=${key} click=${click} wheel=${wheel}`);if(!key||!click||!wheel)console.log(JSON.stringify(out));shell.kill();process.exitCode=key&&click&&wheel?0:1},2400);
